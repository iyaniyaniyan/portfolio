        ...
        <div className="hidden items-center gap-1 lg:flex">
          {navigation.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="rounded-lg px-3 py-2 text-sm font-semibold text-slate-700 transition hover:bg-white/70 hover:text-lagoon"
            >
              {item.label}
            </a>
          ))}
        ...
        <div className="glass-nav mx-auto mt-2 grid max-w-7xl gap-1 rounded-lg p-3 lg:hidden">
          {navigation.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="rounded-lg px-4 py-3 text-sm font-semibold text-slate-800 transition hover:bg-white/75 hover:text-lagoon"
